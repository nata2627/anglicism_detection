...
def merge_csv_files(input_directory, output_file):
    ...
    использует заголовок из первого файла и удаляет пустые строки перед сохранением
    ...
    for i, file in enumerate(csv_files):
        try:
            ...
            if i == 0:
                # Для первого файла сохраняем заголовок
                df = pd.read_csv(file, encoding=encoding)
                header = df.columns.tolist()
                all_dataframes.append(df)
                print(f"Прочитан файл с заголовком: {os.path.basename(file)} (кодировка: {encoding})")
            else:
                # Для остальных файлов используем заголовок из первого файла
                df = pd.read_csv(file, encoding=encoding, names=header, skiprows=1)
                all_dataframes.append(df)
            ...
            for enc in ['utf-8', 'cp1251', 'latin1', 'koi8-r', 'utf-16']:
                try:
                    if i == 0:
                        ...
                        df = pd.read_csv(file, encoding=enc, names=header, skiprows=1)
                    all_dataframes.append(df)
...
if __name__ == "__main__":
    ...
    input_dir = "assets/etalons"

    # Укажите путь к выходному файлу
    output_file = "assets/etalons.csv"
